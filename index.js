...
const PROPOSAL_TOKEN_SECRET =
  process.env.LOOBA_MCP_TOKEN_SECRET || randomBytes(32).toString("hex");
const PROPOSAL_TOKEN_TTL_MS = 30 * 60 * 1000;

function signProposalToken(slugs) {
  const payload = { slugs, ts: Date.now() };
  const json = Buffer.from(JSON.stringify(payload)).toString("base64url");
  const sig = createHmac("sha256", PROPOSAL_TOKEN_SECRET).update(json).digest("base64url");
  return `${json}.${sig}`;
}

function verifyProposalToken(token) {
  if (typeof token !== "string" || !token.includes(".")) {
    return { ok: false, error: "Token is missing or malformed." };
  }
  const [json, sig] = token.split(".");
  const expected = createHmac("sha256", PROPOSAL_TOKEN_SECRET).update(json).digest("base64url");
  if (sig !== expected) {
    return { ok: false, error: "Token signature is invalid (was it tampered with or generated by another server?)." };
  }
  let payload;
  try {
  ...
  if (!payload || !Array.isArray(payload.slugs) || typeof payload.ts !== "number") {
    return { ok: false, error: "Token payload is malformed." };
  }
  if (Date.now() - payload.ts > PROPOSAL_TOKEN_TTL_MS) {
    return { ok: false, error: "Token has expired. Call propose_snippets again to get fresh options." };
  }
  return { ok: true, slugs: payload.slugs };
}
...
server.tool(
  ...
  async ({ query, snippet_type, project_context }) => {
    const ctx = detectFrontendProject();
    ...
    const token = signProposalToken(posts.map((r) => r.slug));

    const lines = [];

    if (ctx.isFrontend) {
      const fwks = ctx.frameworks.length ? ` (${ctx.frameworks.join(", ")}${ctx.cssApproach ? ` + ${ctx.cssApproach}` : ""})` : "";
      lines.push(`Frontend project detected${fwks}.`, "");
    } else {
      lines.push("No frontend project detected in current directory — snippets will still be returned.", "");
    }
    ...
    lines.push(
      "---",
      "**Required workflow:**",
      "1. Present the 3 options above to the user (with their looba.dev links).",
      "2. Ask the user which option they prefer: 1, 2, or 3.",
      "3. Once the user replies, call `integrate_post` with:",
      "   - `slug`: the slug of the chosen option",
      `   - \`user_choice\`: the number they picked (1, 2, or 3)`,
      `   - \`proposal_token\`: \`${token}\``,
      project_context ? `   - \`project_context\`: \`${project_context}\`` : "   - `project_context`: project stack details",
      "",
      "⚠ `integrate_post` will refuse to run without a valid `proposal_token` and `user_choice` that match this proposal."
    );
...
server.tool(
  ...
  "Fetch a Looba snippet and return it with detailed integration instructions " +
    ...
    "REQUIRES a valid proposal_token from a recent propose_snippets call and a user_choice " +
    "matching the slug — the server enforces the 'propose 3, user picks one' workflow.",
  {
    slug: z.string().describe("The slug of the option the user picked (must match user_choice in the proposal_token)"),
    user_choice: z
      .number()
      .int()
      .min(1)
      .max(3)
      .describe("Which option the user picked: 1, 2, or 3 (from the 3 options returned by propose_snippets)"),
    proposal_token: z
      .string()
      .describe("The proposal_token returned by the most recent propose_snippets call. Required."),
    project_context: z
      .string()
      .describe(
  ...
  async ({ slug, user_choice, proposal_token, project_context, target_file }) => {
    const sanitizedSlug = String(slug || "").trim();
    if (!sanitizedSlug) {
      return { content: [{ type: "text", text: "Error: slug is required." }] };
    }

    const tokenResult = verifyProposalToken(proposal_token);
    if (!tokenResult.ok) {
      return {
        content: [
          {
            type: "text",
            text:
              `Refused: ${tokenResult.error}\n\n` +
              "Workflow reminder: call `propose_snippets` first to give the user 3 options, " +
              "ask the user which one they prefer, then call `integrate_post` with the slug, " +
              "the user_choice number (1/2/3), and the proposal_token from that response.",
          },
        ],
      };
    }

    const expectedSlug = tokenResult.slugs[user_choice - 1];
    if (!expectedSlug) {
      return {
        content: [
          {
            type: "text",
            text:
              `Refused: user_choice=${user_choice} is out of range for this proposal ` +
              `(it contained ${tokenResult.slugs.length} option${tokenResult.slugs.length !== 1 ? "s" : ""}).`,
          },
        ],
      };
    }
    if (expectedSlug !== sanitizedSlug) {
      return {
        content: [
          {
            type: "text",
            text:
              `Refused: slug mismatch. user_choice=${user_choice} corresponds to slug "${expectedSlug}" ` +
              `but you passed slug="${sanitizedSlug}". Make sure the slug matches the option the user actually picked.`,
          },
        ],
      };
    }
